...
export class AuthService {
    constructor(@InjectModel(Users.name) private userModel : Model<Users>,
        ...
        @InjectModel(BlacklistToken.name) private blacklistTokenModel: Model<BlacklistToken>
    ){
  ...
  if (!existingUser) {
    throw new HttpException('No User Exist in this email', HttpStatus.NOT_FOUND);
  }

  const isSamePassword = await bcrypt.compare(userData.newPassword, existingUser.password);
  if (isSamePassword) {
  ...
  return { token: this.jwtService.sign(payload) };
    ...
    async logout(data): Promise<any>{
        ...
        try{
            const newBlacklistedToken = new this.blacklistTokenModel({token: data.token});
